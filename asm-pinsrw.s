...
#	push		%r15
	...
	mov			%rdi, %rbp	# dest buffer.  frees up rdi for more frequently-used addresses
...
# %rdi: lookup table (one less byte to encode than %rbp).  swapping thse regs seems to be a very small slowdown on SnB.
...
.loop:
	...
	pinsrw		$2, 0x0400(%rdi, %rbx, 4), %xmm1
	 ...
	 movzx		%ch, %ebx
	 pinsrw		$1, 0x0000(%rdi, %rax, 4), %xmm2
	 shr			$16, %rcx
	movzx		%dl, %eax
	 pinsrw		$1, 0x0400(%rdi, %rbx, 4), %xmm3
	...
	pinsrw		$3, 0x0400(%rdi, %rbx, 4), %xmm1
	 ...
	 pinsrw		$2, 0x0000(%rdi, %rax, 4), %xmm2
	 pinsrw		$2, 0x0400(%rdi, %rbx, 4), %xmm3
	 movzx		%cl, %eax
	 movzx		%ch, %ebx
	pxor		%xmm1, %xmm0
	 pinsrw		$3, 0x0000(%rdi, %rax, 4), %xmm2
	 pinsrw		$3, 0x0400(%rdi, %rbx, 4), %xmm3
